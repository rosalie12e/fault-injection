// standard package for handling errors
package helper
...
// Note - for ease of use, failure of this fault-injection module does not cause failure of the NDC connector.
...
func ValidateErrorCode(code, title, description string, isThirdPartyError bool) {
	...
	DataDogHandle.LogDebug("errorHandler setup: ", Config) //debug statement
	...
	if strings.HasPrefix(code, "TFM_") {
		...
		tfmCustomCode = "TFM_2033" //standard Fault Injection module error code
	}

	//assigns an error severity depending on where the TFM_ error code appears in the WebServiceAPIErrorsMap.
	...
	//log the error and add metrics depending on error severity
